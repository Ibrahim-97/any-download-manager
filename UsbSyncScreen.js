...
export default function UsbSyncScreen() {
  ...
  const [databaseCleanupSuccess, setDatabaseCleanupSuccess] = useState(false)

  const [databaseCleanupDeleted, setDatabaseCleanupDeleted] = useState({})
...
* التحقق من جميع جداول Database Sync
*
* syncFiles هنا Metadata خاصة بمزامنة الملفات.
* لا يتم حذفها بعد نجاح نقل الملف.
  */
  ...
  const handleDatabaseSyncComplete = useCallback(
    async message => {
      try {
        ...
        /*
         * مهم:
         * لا يتم تحديث lastSyncAt إلا بعد نجاح
         * تطبيق البيانات بالكامل.
         */
    ...
    async approvedRows => {
      ...
      await db.transaction(async tx => {
        for (const tableName of deleteOrder) {
          ...
          deleted[tableName] = Number(result?.changes || 0)
        }
      })
  ...
  const startDatabaseCleanup = useCallback(async () => {
    if (!connectedRef.current || !usbConnected) {
      Alert.alert("تنظيف قاعدة البيانات", "الكمبيوتر غير متصل عبر USB.")
    ...
    const peerId = pcDeviceRef.current?.id || null

    if (!peerId) {
      Alert.alert("تنظيف قاعدة البيانات", "معرف الكمبيوتر غير متاح.")
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.log("USB DATABASE CLEANUP START")
              ...
              if (!success) {
                throw new Error("DATABASE_CLEANUP_REQUEST_SEND_FAILED")
              }
            } catch (error) {
              console.error("USB DATABASE CLEANUP START ERROR:", error)
              ...
              setUsbError(error?.message || "فشل بدء تنظيف قاعدة البيانات.")
            }
          },
        },
      ],
    )
  }, [getSoftDeletedDatabaseRows, send, trusted, usbConnected])
  ...
  const handleDatabaseCleanupPlan = useCallback(
    async message => {
      try {
        ...
        const cleanupRequest = databaseCleanupRequestRef.current
        ...
        if (
          ...
          console.warn("USB DATABASE CLEANUP PLAN REQUEST ID MISMATCH", {
            localRequestId: cleanupRequest.requestId,
        ...
        console.log("USB DATABASE CLEANUP PLAN RECEIVED")
        ...
        if (commonCount === 0) {
          ...
          console.log("USB DATABASE CLEANUP FINISHED - NO COMMON ROWS")
        ...
        if (!success) {
          throw new Error("DATABASE_CLEANUP_COMMIT_SEND_FAILED")
        }

        console.log("USB DATABASE CLEANUP COMMIT SENT")

        console.log("========================================")

        console.log("USB DATABASE CLEANUP WAITING FOR REMOTE DONE")
        ...
        console.error("USB DATABASE CLEANUP PLAN ERROR:", error)
        ...
        setUsbError(error?.message || "فشل معالجة خطة تنظيف قاعدة البيانات.")
      }
    },
    [buildCommonCleanupRows, send],
  )

  const handleDatabaseCleanupRemoteDone = useCallback(
    async message => {
      try {
        const payload = message?.payload || {}
        ...
        if (
          ...
          Number(cleanupRequest?.commonCount || 0) === 0
        ) {
          console.log("========================================")

          console.log("USB DATABASE CLEANUP REMOTE DONE - NO COMMON ROWS")
        ...
        if (
          ...
          console.warn("USB DATABASE CLEANUP REMOTE DONE REQUEST ID MISMATCH", {
            localRequestId: cleanupRequest.requestId,
            remoteRequestId: requestId,
          })
        ...
        if (Number(cleanupRequest.commonCount || 0) === 0) {
          console.log("========================================")
          ...
          console.log({
            requestId,
            commonCount: cleanupRequest.commonCount,
          })
        ...
        const approvedRows =
          payload?.approvedRows || cleanupRequest.commonRows || {}

        const deleted = await permanentlyDeleteLocalRows(approvedRows)

        setDatabaseCleanupDeleted(deleted)
        ...
        console.log("USB DATABASE CLEANUP REMOTE DONE")
        ...
        if (!success) {
          throw new Error("DATABASE_CLEANUP_COMPLETE_SEND_FAILED")
        }

        console.log("USB DATABASE CLEANUP COMPLETE SENT")
      } catch (error) {
        console.error("USB DATABASE CLEANUP REMOTE DONE ERROR:", error)
        ...
        setUsbError(error?.message || "فشل إتمام تنظيف قاعدة البيانات.")
      }
    },
    [permanentlyDeleteLocalRows, send],
  )

  const handleDatabaseCleanupFinished = useCallback(
    async message => {
      try {
        const payload = message?.payload || {}
        ...
        if (
          ...
          console.warn("USB DATABASE CLEANUP FINISHED REQUEST ID MISMATCH", {
            localRequestId: cleanupRequest.requestId,
            remoteRequestId: requestId,
          })
        ...
        if (payload?.success !== true) {
          throw new Error(
            payload?.error || payload?.message || "DATABASE_CLEANUP_FAILED",
          )
        }

        const deleted = payload?.deleted || databaseCleanupDeleted || {}
        ...
        console.log("USB DATABASE CLEANUP FINISHED SUCCESSFULLY")
        ...
        console.error("USB DATABASE CLEANUP FINISHED ERROR:", error)
        ...
        setUsbError(error?.message || "فشل إنهاء تنظيف قاعدة البيانات.")
      }
    },
  ...
  const downloadIncomingFile = useCallback(
    async data => {
      ...
      /*
       * إذا أرسل الكمبيوتر URL كاملًا نستخدمه كما هو.
       * وإذا أرسل مسارًا فقط نضيف عنوان HTTP الخاص بـ USB.
       */
      const normalizedUrl =
      ...
      try {
        ...
        if (result && typeof result.then === "function") {
          result.catch(error => {
            ...
            markTransferError(requestId, error?.message || "تعذر تنزيل الملف.")
          })
        }
      } catch (error) {
...
*
* مهم جدًا:
* لا نحذف syncFiles هنا.
*
* syncFiles هو سجل المزامنة وخصوصًا عند وجود
* deleted_at يجب أن يبقى حتى تتم عملية
* Database Cleanup على الجهازين.
  */
  const markTransferCompleted = useCallback(
    async requestId => {
      ...
      /*
       * لا يوجد delete من syncFiles هنا.
       *
       * سابقًا كان الكود يحذف:
       *
       * await db.delete(schema.syncFiles)...
       *
       * وهذا غير صحيح مع نظام الـ soft-delete
       * والمزامنة الحالية.
       */
      ...
      if (
        ...
        console.log(
          "USB DATABASE FILE TRANSFER COMPLETED - syncFileId PRESERVED:",
          String(pendingFile.syncFileId),
        )
      }
    ...
    [resolveAutoSyncWaiter, updateTransfer],
  )
  ...
  const sendDatabaseFile = useCallback(
    async file => {
      ...
      try {
        ...
        markTransferError(requestId, error?.message || "تعذر إرسال طلب الملف.")
    ...
    async requests => {
      ...
      for (const request of requests) {
        try {
          ...
          /*
           * entityId يتم استخراجه من أول جزء
           * في المسار:
           *
           * entityId/fileName
           */
  ...
  const handleMessage = useCallback(
    async incoming => {
      ...
      if (type === "PAIR_CODE") {
        ...
        /*
         * يتم حفظ الكود في state فقط
         * لإدخاله يدويًا من المستخدم.
         */
      ...
      if (type === "DATABASE_CLEANUP_PLAN") {
        await handleDatabaseCleanupPlan(message)

        return
      }

      if (type === "DATABASE_CLEANUP_REMOTE_DONE") {
        await handleDatabaseCleanupRemoteDone(message)

        return
      }

      if (type === "DATABASE_CLEANUP_FINISHED") {
        await handleDatabaseCleanupFinished(message)

        return
      }

      if (type === "DATABASE_CLEANUP_ERROR") {
        console.error("USB DATABASE CLEANUP ERROR:", payload)
      ...
      if (type === "FILE_SEND_REQUEST") {
        ...
        /*
         * مهم:
         * لا نستخدم pendingFile هنا قبل إنشائه.
         * نستخدم البيانات القادمة من الرسالة.
         */
      ...
      if (type === "FILE_ACCEPT") {
        ...
        /*
         * تحويل المسار النسبي الذي يرجعه
         * الكمبيوتر إلى URL HTTP صالح.
         */
        const normalizedUrl = /^https?:\/\//i.test(String(uploadUrl))
          ? String(uploadUrl)
          : `http://${USB_HOST}:${USB_HTTP_PORT}${
...
* تصحيح dependencies الخاصة بإعادة الاتصال:
* connectUsb يتم تعريفه قبل أول استدعاء فعلي
* لـ scheduleUsbReconnect.
  */
  useEffect(() => {
    /*

  * هذا effect لا يقوم بأي شيء.
  * وجوده فقط يحافظ على ترتيب hooks واضح
  * ولا يغير سلوك الشاشة.
    */
  }, [])
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        const messageSubscription = AvocatoFlow.addListener(
          "onMessage",
          event => {
        ...
        addSubscription(messageSubscription)

        if (!active) {
          return
        }

        const stateSubscription = AvocatoFlow.addListener(
          "onConnectionStateChanged",
          handleConnectionState,
        )

        addSubscription(stateSubscription)

        if (!active) {
          return
        }

        const errorSubscription = AvocatoFlow.addListener(
          "onConnectionError",
        ...
        const fileProgressSubscription = AvocatoFlow.addListener(
          "onFileProgress",
          event => {
            if (!active) {
              return
        ...
        addSubscription(fileProgressSubscription)

        if (!active) {
          return
        }

        const fileCompletedSubscription = AvocatoFlow.addListener(
          "onFileCompleted",
          event => {
            if (!active) {
              return
            ...
            if (pendingFile?.direction === "ANDROID_TO_PC") {
              updateTransfer(requestId, {
                transferId,
                transferred: Number(pendingFile.size || 0),
                total: Number(pendingFile.size || 0),
                progress: 1,
                status: "completed",
              })

              markTransferCompleted(requestId).catch(error => {
                console.error("USB NATIVE UPLOAD COMPLETION ERROR:", error)
              })
        ...
        addSubscription(fileCompletedSubscription)

        if (!active) {
          return
        }

        const fileErrorSubscription = AvocatoFlow.addListener(
          "onFileError",
          event => {
            if (!active) {
              return
        ...
        addSubscription(fileErrorSubscription)
...
* المزامنة التلقائية:
*
* Database Sync و Case Files Sync يبدأان
* بنفس الدورة كما في التصميم السابق.
  */
  useEffect(() => {
    if (!usbConnected) {
      return
  ...
  }, [usbConnected, trusted, startDatabaseSync, startCaseFilesSync])
    ...
    >
      ...
      >
        ...
        <View style={styles.card}>
            ...
            <View style={styles.connectedBox}>
              ...
              <View style={styles.deviceInfo}>
                ...
                <Text style={styles.deviceIp}>
                  WS:{USB_WS_PORT} HTTP:{USB_HTTP_PORT}
                </Text>
          ...
          {!usbConnected ? (
            <Pressable style={styles.primaryButton} onPress={connectUsb}>
              {usbStatus === "connecting" ? (
